...
for (const table of missing) {
  ...
  if (startLine === -1) {
    // Try view
    const viewPattern = `-- View structure for ${table}`;
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes(viewPattern)) { startLine = i; break; }
    }
  }
  ...
  for (let i = startLine + 1; i < lines.length; i++) {
    if (lines[i].includes('-- Table structure for') || lines[i].includes('-- View structure for') || lines[i].includes('-- Dumping data for')) {
      endLine = i;
      break;
    }
  }
  
  // Only take DROP + CREATE statements, skip INSERT data
  ...
  allSql += filtered.join('\n') + '\n';
  console.log(`+ ${table}`);
}

const tempFile = process.env.TEMP + '/create_tables_v2.sql';
...
try {
  ...
  console.log('Errors:', (e.stderr || e.message).substring(0, 500));
}
